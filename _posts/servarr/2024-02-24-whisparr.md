...
tags:
  ...
  - whisparr
---
![Whisparr Icon](https://wiki.servarr.com/assets/whisparr/logos/128.png)
...
SSH into your server. Depending on how you want to set it up you can create a folder called docker and nest your docker compose folders inside like /docker/whisparr or just create a folder called whisparr. Then cd into it.

For /docker/whisparr do this...
``` sh
sudo mkdir docker && cd docker
```
then
``` sh
sudo mkdir whisparr && cd whisparr
```
or for just a whisparr directory like /whisparr do this...
``` sh
sudo mkdir whisparr && cd whisparr
```
...
Now that you have it running, open your favorite web browser and go to http://localhost:6969 if running on the same machine, or if it is not, use the ip address of the server. 

You now have a running version of whisparr!!
